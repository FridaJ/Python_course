...
NMRdata = pd.read_csv('NMR_for_python_fixed.csv')
NMR_df = pd.DataFrame(NMRdata)   # csv -> df
NMR_df = NMR_df.drop([0], axis=0)   # dropped extra header row
...
i_shift1 = col_index(NMR_df, "Shift1") # index 5
i_shiftC1 = col_index(NMR_df, "ShiftC1") # index 328
i_shiftLast = len(NMR_df.columns)-1 # index 650
# Use columns with index 5 to 327 for uncorrected values and 328 to 650 for corrected values

# Keep only data from NMR shifts in df and divide into uncorrected and corrected data
Temp4_uncorr = Temp4.iloc[:, 5:328]
Temp4_corr = Temp4.iloc[:, 328:651]
Temp25_uncorr = Temp25.iloc[:, 5:328]
Temp25_corr = Temp25.iloc[:, 328:651]
...
sign_diff_uncorr = sign_diff_check(Temp4_uncorr, Temp25_uncorr, 0.05)  # a list of column names
...
sign_diff_corr = sign_diff_check(Temp4_corr, Temp25_corr, 0.05)
...
# Get indices or column names for the columns to be used for analysis (get from function sign_diff_check?)
